package com.tkvoid.tags.test.hbase.write
...
object HBaseWriteTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tagsRDD: RDD[(String, String)] = sc.parallelize(
      List(("1001", "gender:男,job:教师"), ("1002", "gender:女,job:工人"),
      ("1003", "gender:男,job:学生"), ("1004", "gender:男,job:工人")
      ),
      numSlices = 2
    )

    // TODO: 将RDD数据保存到HBase表中， 要求RDD数据类型为二元组，Key： ImmutableByteWritable， Value： Put
    /*
    Hbase 表: htb_tags
            RowKey: userId
            CF: user
    ...
    val datasRDD: RDD[(ImmutableBytesWritable, Put)] = tagsRDD.map{case(userId, tags) =>
                  ...
                  tags.split(",").foreach{
                    tag => val Array(field, value) = tag.split(":")
                      put.addColumn(
